...
4. **Histéresis:**  
   *"La histéresis evita el problema de chattering: si usáramos un solo umbral de 55%, la bomba se encendería y apagaría repetidamente si la humedad oscila entre 53% y 57%. Con histéresis, la bomba se enciende cuando la humedad baja de 55% pero solo se apaga cuando supera el 70%, dando estabilidad al sistema. La banda de histéresis es de 15%."*
...
*"En lugar de un único umbral (ej: 55%), usamos dos: uno para encender (55%) y otro para apagar (70%). El sistema solo puede encender la bomba si la humedad está por debajo del umbral inferior (55%), y solo puede apagarla si está por encima del umbral superior (70%). Entre ambos valores, el estado se mantiene. La diferencia de 15% entre umbrales es la banda de histéresis."*
...
*"El sensor tiene una respuesta inversa: a mayor humedad, menor valor ADC. Medimos el valor ADC con el sensor en seco (~4100) y sumergido en agua (~1200), y usamos una interpolación lineal (`map()`) para convertir cualquier valor ADC a porcentaje de humedad. Estos valores están definidos en `config.h` como `SOIL_ADC_SECO` y `SOIL_ADC_MOJADO`. El ESP32 usa ADC de 12 bits (0–4095), por lo que los valores son distintos a los de un Arduino Uno (10 bits, 0–1023)."*
