...
Humedad (%)
    │
 45 ┤────────────────────── UMBRAL_MAX ─── Apagar bomba
    │              ╔═══════╗
 30 ┤────── UMBRAL_MIN ─────╚═══════════── Encender bomba
    │      ║               
    │══════╝               
    ├──────────────────────────────► Tiempo
    │
    Zona de histéresis: [30%, 45%]
...
case IrrigationState::IDLE:
    if (data.humedadSustrato < UMBRAL_HUMEDAD_MIN) {  // < 30%
...
case IrrigationState::WATERING:
    if (data.humedadSustrato > UMBRAL_HUMEDAD_MAX) {  // > 45%
