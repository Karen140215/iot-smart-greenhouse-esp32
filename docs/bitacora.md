...
**Procedimiento:**
1. Con el sensor completamente seco (en el aire), leer el ADC: valor = **4100** → definido como `SOIL_ADC_SECO`
2. Con el sensor sumergido en agua, leer el ADC: valor = **1200** → definido como `SOIL_ADC_MOJADO`
3. Actualizar `SOIL_ADC_SECO` y `SOIL_ADC_MOJADO` en `config.h` si los valores difieren con el hardware disponible.

**Resultado:** Mapeo lineal calibrado entre ADC y porcentaje de humedad.  
**Observación importante:** El ADC del ESP32 tiene ligeras no-linealidades cerca de 0V y 3.3V. Se promedian 5 muestras para reducir el ruido. Para mayor precisión, se puede usar una corrección por tabla de búsqueda (look-up table). Nota: los rangos ADC del ESP32 (12 bits, 0–4095) son aproximadamente 4 veces más grandes que los del Arduino Uno (10 bits, 0–1023); los valores de calibración de Arduino no son válidos aquí.
...
**Prueba:** Se simuló la variación de humedad alrededor del umbral inferior (55%) con valores de prueba. Sin histéresis, la bomba se encendería y apagaría continuamente si la humedad oscila entre 53% y 57%. Con histéresis (umbral de apagado en 70%), la bomba permanece encendida hasta alcanzar el 70%, evitando el chattering. La banda de histéresis es de 15% (`HISTERESIS_HUMEDAD`).
...
- [x] Activación automática de bomba cuando humedad < 55%
- [x] Desactivación de bomba cuando humedad > 70%
